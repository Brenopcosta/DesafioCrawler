	...
	public void executarCrawler() {
		...
		for (WebElement elemento : listaDeElementosDeNoticia) {
			...
			data = noticia.findElement(By.tagName("time")).getText();
				...
				try{
					data = noticia.findElement(By.className("playlist__details__datetime")).getText();
				}
				catch (Exception x) {
					try {
						data= noticia.findElement(By.className("live-at")).getText();
					}catch (Exception z) {
						noticia.close();
						continue;					
					}
			...
			try {
			List<WebElement> listaDeElementosComTexto = noticia.findElements(By.className("content-text__container"));
			...
			for(int i = 1 ; i < listaDeElementosComTexto.size();i++) {
				...
				try {
				List<WebElement> listaDeElementosComTexto = noticia.findElements(By.className("cb-summary__item"));
				texto = listaDeElementosComTexto.get(0).getText();
